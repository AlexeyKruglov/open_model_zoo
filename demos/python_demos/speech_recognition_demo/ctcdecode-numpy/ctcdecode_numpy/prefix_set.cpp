...
size_t PrefixSet::add_words(const std::vector<std::vector<int> >& words) {
  ...
  // Go through the sorted list, and maintain the stack of PrefixSetStates for
  // the prefixes of the last word to initialize trie_starts_.
...
PrefixSetState PrefixSet::empty_state() {
  PrefixSetState empty;
  empty.length = 0;
  empty.start = 0;
...
bool PrefixSet::append_character(int character, PrefixSetState& state) {
